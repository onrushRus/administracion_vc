...
abstract class BaseComentario extends BaseObject  implements Persistent
{
	...
	public function setSocioId($v)
	{
		...
		if ($this->socio_id !== $v) {
			$this->socio_id = $v;
			$this->modifiedColumns[] = ComentarioPeer::SOCIO_ID;
		}
	...
	public function setPeliculaId($v)
	{
		...
		if ($this->pelicula_id !== $v) {
			$this->pelicula_id = $v;
			$this->modifiedColumns[] = ComentarioPeer::PELICULA_ID;
		}
	...
	public function setDetalle($v)
	{
		...
		if ($this->detalle !== $v) {
			$this->detalle = $v;
			$this->modifiedColumns[] = ComentarioPeer::DETALLE;
		}
	...
	public function setFecha($v)
	{
		...
		if ($this->fecha !== null || $dt !== null) {
			...
			if ($currentDateAsString !== $newDateAsString) {
				...
				$this->modifiedColumns[] = ComentarioPeer::FECHA;
			}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 5; // 5 = ComentarioPeer::NUM_HYDRATE_COLUMNS.
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			// symfony_behaviors behavior
			foreach (sfMixer::getCallables('BaseComentario:save:pre') as $callable)
			{
			...
			if ($ret) {
				...
				$this->postSave($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseComentario:save:post') as $callable)
				{
	...
	protected function doInsert(PropelPDO $con)
	{
		...
		if ($this->isColumnModified(ComentarioPeer::SOCIO_ID)) {
			$modifiedColumns[':p' . $index++]  = '`SOCIO_ID`';
		}
		if ($this->isColumnModified(ComentarioPeer::PELICULA_ID)) {
			$modifiedColumns[':p' . $index++]  = '`PELICULA_ID`';
		}
		if ($this->isColumnModified(ComentarioPeer::DETALLE)) {
			$modifiedColumns[':p' . $index++]  = '`DETALLE`';
		}
		if ($this->isColumnModified(ComentarioPeer::FECHA)) {
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(ComentarioPeer::SOCIO_ID)) $criteria->add(ComentarioPeer::SOCIO_ID, $this->socio_id);
		if ($this->isColumnModified(ComentarioPeer::PELICULA_ID)) $criteria->add(ComentarioPeer::PELICULA_ID, $this->pelicula_id);
		if ($this->isColumnModified(ComentarioPeer::DETALLE)) $criteria->add(ComentarioPeer::DETALLE, $this->detalle);
		if ($this->isColumnModified(ComentarioPeer::FECHA)) $criteria->add(ComentarioPeer::FECHA, $this->fecha);
	...
	public function getPelicula(PropelPDO $con = null)
	{
		if ($this->aPelicula === null && ($this->pelicula_id !== null)) {
			...
			/* The following can be used additionally to
				...
				undesirable since it could result in an only partially populated collection
				in the referenced object.
				$this->aPelicula->addComentarios($this);
			 */
		}
	...
	public function __call($name, $params)
	{
		...
		// symfony_behaviors behavior
		if ($callable = sfMixer::getCallable('BaseComentario:' . $name))
		{
